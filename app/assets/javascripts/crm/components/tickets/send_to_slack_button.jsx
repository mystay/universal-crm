var SendToSlackButton = React.createClass({
  ...
  render: function(){
    if (this.props.gs.users.length>0 && this.props.gs.user.functions.indexOf("send_to_slack") > -1){
      return(
        <div>
          <button className="btn btn-info btn-xs" onClick={this.sendToSlack}>
  ...
  sendToSlack: function(){
    $.ajax({
      method: 'GET',
      url: `/crm/tickets/${this.props.ticket.id}.json`,
      success: (function(data){
        console.log(data.ticket.comments.slice(-1)[0].content )
      }).bind(this)
    });
