...
module UniversalCrm
  class ConfigController < ApplicationController
    ...
    def update
      p = params.require(:config).permit(:system_name, :inbound_domain, :inbound_email_address, :transaction_email_address, :transaction_email_from, :new_ticket_header, :new_reply_header, :email_footer, :ticket_flags)
      p[:ticket_flags] = p[:ticket_flags].to_s.gsub('\r','').split("\n").map{|p| {label: p.split('|')[0], color: p.split('|')[1]}}
      p[:inbound_email_address] = p[:inbound_email_address].downcase.strip
      puts p
